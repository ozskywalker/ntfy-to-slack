...
func TestPrintHelp_Output(t *testing.T) {
	...
	// Verify output contains expected information. The Flags section is
	// generated by config.FlagUsage() from the actual flag.FlagSet (see
	// config.buildFlagSet), which renders flag names with a single leading
	// dash (Go's flag package accepts both "-x" and "--x" when parsing, but
	// only ever prints the single-dash form itself).
