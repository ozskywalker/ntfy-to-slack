...
// These cases originally lived in cmd/ntfy-to-slack's test file under the
// theory that they exercised "what main does", but they only ever call
// config.New/Validate directly ("Test configuration parsing directly since
// we can't easily test main() exit codes" -- a comment from the original),
// so they're config tests in every way that matters and belong here.
